...
static const std::string prefix = "/Users/atulkumar/mine/KDTree_extension/digit-classifier/";
...
bool MainWindow::LoadingThread::loadDataSet(KDTree<kImageSize, unsigned char>& kd) try {
  ...
  images.open(prefix + "training-images", ios::binary);
  
  ifstream labels; // Labels for those images
  labels.exceptions(ios::failbit | ios::badbit);
  labels.open(prefix + "training-labels", ios::binary);
