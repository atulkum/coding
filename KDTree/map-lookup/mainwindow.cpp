...
static const std::string prefix = "/Users/atulkumar/mine/KDTree_extension/map-lookup/";
    ...
    ui(new Ui::MainWindow)
{
  ...
  worldMapPic = new PictureDisplay(prefix + "world-map.bmp");
...
bool MainWindow::LoadingThread::loadGeoCodes(map<string, string>& geoLookup) {
  ...
  ifstream geoCodes(prefix + "geo-codes.txt", ios::binary);
...
bool MainWindow::LoadingThread::loadGeographicData(KDTree<2, string>& kd) {
  ...
  ifstream input(prefix + "place-data.txt");
